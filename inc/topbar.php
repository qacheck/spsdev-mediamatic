...
/**
 * class Spsdev_Mediamatic_Topbar
 * the main class
 */
class Spsdev_Mediamatic_Topbar {

	
	public $plugin_version = SPSDEV_MEDIAMATIC_VERSION;
	...
	{
        $spsdev_mediamatic_Folder = isset($_REQUEST["themedoWMCFolder"]) ? sanitize_text_field($_REQUEST["themedoWMCFolder"]) : null;
        if (is_null($spsdev_mediamatic_Folder)) 
		{
            $spsdev_mediamatic_Folder = isset($_REQUEST["themedo_spsdev_mediamatic_folder"]) ? sanitize_text_field($_REQUEST["themedo_spsdev_mediamatic_folder"]) : null;
        }
        if ($spsdev_mediamatic_Folder !== null) 
		{
            $spsdev_mediamatic_Folder = (int)$spsdev_mediamatic_Folder;
            if ($spsdev_mediamatic_Folder > 0) {
                wp_set_object_terms($post_ID, $spsdev_mediamatic_Folder, SPSDEV_MEDIAMATIC_FOLDER, false);
            }
        }
    }
    ...
    public static function mediamaticGetTermsValues( $keys = 'ids' ) 
	{
        ...
        // Get media taxonomy
        $media_terms = get_terms( SPSDEV_MEDIAMATIC_FOLDER, array(
            'hide_empty' => 0,
            'fields'     => 'id=>slug',
        ) );
        $media_values = array();
    ...
    public function mediamaticSaveMultiAttachments()
	{
		...
		{
            $term_list 	= wp_get_post_terms( sanitize_text_field($id), SPSDEV_MEDIAMATIC_FOLDER, array( 'fields' => 'ids' ) );
            $from 		= -1;
            ...
            wp_set_object_terms( $id, intval(sanitize_text_field($_REQUEST['folder_id'])), SPSDEV_MEDIAMATIC_FOLDER, false );
    ...
    public function mediamaticGetTermsByAttachment()
	{
        ...
        $terms  = get_the_terms($id, SPSDEV_MEDIAMATIC_FOLDER);
...
$spsdev_mediamatic_topbar = new Spsdev_Mediamatic_Topbar();
