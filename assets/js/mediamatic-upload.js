...
(function( $ ) {
	...
	Core = {
		init : function(){
			html = '';
			html += $("#spsdev_mediamatic_sidebar").html();
			$("#spsdev_mediamatic_sidebar").remove();
			...
			$(".panel-left").mediamaticResize({
				handleSelector: ".spsdev_mediamatic_splitter",
				resizeHeight: false,
				onDrag: function (e, $el, newWidth, newHeight, opt) {
				 // limit box size
				 	...
				 	if (newWidth < themedoMinWidth){
						...
						$('.spsdev_mediamatic_sidebar_fixed').css('max-width', '0');
						$('.spsdev_mediamatic_sidebar_fixed').css('overflow', 'hidden');
						var $wrapAll = $('.wrap-all');
				 		$( ".spsdev_mediamatic_splitter").hide() 
				 		$wrapAll.addClass('show-expand');
				 		if(!$('.wrap-all >  span').length){
				 			$('.wrap-all').prepend("<span class='spsdev_mediamatic_sidebar_expand_button'></span>");
				 		}
				 		setTimeout(function(){
				 			$('.spsdev_mediamatic_sidebar_expand_button').show();
				 		}, 600);	
			 			

			 			$('.spsdev_mediamatic_sidebar_expand_button').on('click', function(){
							
							$(this).hide();
							$('.spsdev_mediamatic_sidebar').css({'max-width': '800px', 'width': themedoMinWidth + 5+ 'px'});
							$('.spsdev_mediamatic_sidebar_fixed').css({'max-width': '800px', 'width': themedoMinWidth + 5+ 'px'});
							$('.spsdev_mediamatic_splitter').show();
			...
			$('.spsdev_mediamatic_sidebar, .spsdev_mediamatic_splitter, #wpbody .wrap').wrapAll('<div class="wrap-all"></div>');
		},
				...
				if ($("#themedo-mediamatic-defaultTree").length){
					...
					$('#themedo-mediamatic-defaultTree').on("changed.jstree", function (e, data) {
						...
						if($('.menu-item.current_folder').length){
							if (!$('select[name="themedo_spsdev_mediamatic_folder"]').length) {//grid list
		...
		toolbar: {
			...
			create: function(){
				if ($(".js_spsdev_mediamatic_create").length){
					$(".js_spsdev_mediamatic_create").on("click",function(){
			...
			delete: function(){
				...
				if ($(".js_spsdev_mediamatic_delete").length){
					$(".js_spsdev_mediamatic_delete").on("click",function(){
		...
		tooltip : { 
			init: function(){
				if ($(".js_spsdev_mediamatic_tipped").length){
					Tipped.create(".js_spsdev_mediamatic_tipped",function(element){
						return {
