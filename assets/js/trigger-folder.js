...
(function ($) {
	...
	themedo_trigger_folder.rename = function (current, new_name) {
		...
		$.post(ajaxurl, jdata, function (response) {
			...
			if (response == 'error') {
				...
				Popup.errorPopup(spsdev_mediamatic_translate.this_folder_is_already_exists);

			}

		}).fail(function () {
			
			Popup.errorPopup(spsdev_mediamatic_translate.error_occurred);
	...
	themedo_trigger_folder.delete = function (current) {
		...
		var data = {
			'action': 'spsdev_mediamatic_ajax_delete_folder_list',
			'current': current,
		};
		//2. Delete folder
		$.post(ajaxurl, data, function (response) {
			if (response == 'error') {
				...
				Popup.errorPopup(spsdev_mediamatic_translate.folder_cannot_be_delete);
			}
	...
	themedo_trigger_folder.update_folder_position = function () {
		...
		var data = {
			'action': 'spsdev_mediamatic_ajax_update_folder_position',
			'result': result
		}
		...
		$.post(ajaxurl, data, function (response) {
			if (response == 'error') {
				...
				var text = spsdev_mediamatic_translate.something_not_correct + spsdev_mediamatic_translate.this_page_will_reload;
	...
	themedo_trigger_folder.getChildFolder = function (folder_id) {
		...
		var data = {
			'action': 'spsdev_mediamatic_ajax_get_child_folders',
			'folder_id': folder_id,
		};
	...
	$(document).on('click', '.spsdev_mediamatic_sidebar .jstree-anchor', function () {
		...
		if (themedo_clicks !== false) {
			...
			if ($('select[name="themedo_spsdev_mediamatic_folder"]').length) {//list mode
				$('select[name="themedo_spsdev_mediamatic_folder"]').val(folder_id);
				if ($('.spsdev_mediamatic_be_loader').hasClass('loading')) {
					return;
				}
				mediamaticWMC.spsdev_mediamatic_begin_loading();
...
//			$('.js_spsdev_mediamatic_rename').trigger('click');  //perform double-click action
...
//		}
	...
	$(document).on('click', '.pagination-links a', function (event) {
		...
		$.ajax({
			...
			.done(function (html) {
				...
				themedo_after_loading_media(html, $('select[name="themedo_spsdev_mediamatic_folder"]').val());
			})
			.fail(function () {
				mediamaticWMC.spsdev_mediamatic_finish_loading();
				console.log("error");
	...
	function themedo_after_loading_media(html, folder_id) {
		...
		//set folder select
		$.each(spsdev_mediamatic_taxonomies.folder.term_list, function (index, el) {
		...
		var text_drag = spsdev_mediamatic_translate.move_1_file;
		$.each($('table.wp-list-table tr'), function (index, el) {
			$(el).drag("start", function () {
				...
				if (selected_files.length > 0) {
					text_drag = spsdev_mediamatic_translate.Move + ' ' + selected_files.length + ' ' + spsdev_mediamatic_translate.files;
				}
				...
				.drag("end", function () {
					...
					text_drag = spsdev_mediamatic_translate.move_1_file;
				})
				.drag(function (ev, dd) {
					var id = $(this).attr("id");
					id = id.match(/post-([\d]+)/);
