...
var themedo_spsdev_mediamatic_pro_link = 'http://mediamatic.frenify.com/1/';
...
(function ($){
    ...
    var MediamaticCoreTree = {
        ...
        options: {
            ...
            globalMaxDepth: Number.MAX_SAFE_INTEGER,
            sortableItems: '> *',
            targetTolerance: 0
        },
        ...
        init: function () {
			...
			$('.spsdev_mediamatic_toolbar').remove();
		...
		createActions: function(){
			var html = '<div class="spsdev_mediamatic_actions_content"><ul><li class="kaku"><span class="add_new">'+spsdev_mediamatic_translate.new_folder+'</span></li><li><span class="rename">'+spsdev_mediamatic_translate.rename+'</span></li><li><span class="delete">'+spsdev_mediamatic_translate.delete+'</span></li></ul></div>';
			$('body').append(html);
		},
		...
		addActionsContent: function(){
			...
			$('.menu-item .action_button').on('click', function(e){
				...
				var content = $('.spsdev_mediamatic_actions_content');
				var li 		= el.parent();
				var id 		= li.data('id');
			...
			$(window).on('click', function(){
				$('.spsdev_mediamatic_actions_content').removeClass('active').attr('data-id', '');		 
			});
		...
		imgToSvg: function(){
			$('img.spsdev_mediamatic_be_svg').each(function(){
				var $img 		= $(this);
        ...
        themedo_spsdev_mediamatic_upgrade_options: function () {
            var options = {
                title: spsdev_mediamatic_translate.notice,
                html: spsdev_mediamatic_translate.limit_folder,
                type: "warning",
                showCancelButton: true,
                confirmButtonText: spsdev_mediamatic_translate.ok,
                cancelButtonText: spsdev_mediamatic_translate.no_thank,
		...
		fireActions: function(){
			...
			// add new
			$('.spsdev_mediamatic_actions_content span.add_new').on('click', function(){
				
				// light version limitation
                if ($('#folders-to-edit li').length >= Number.MAX_SAFE_INTEGER) {
			...
			// rename
			$('.spsdev_mediamatic_actions_content span.rename').on('click', function(){
			...
			// delete
			$('.spsdev_mediamatic_actions_content span.delete').on('click', function(){
		...
		deleteFolderOops: function(){
			...
			var html = '<div id="spsdev_mediamatic_be_confirm">';
					html += '<div class="confirm_inner">';
						html += '<div class="desc_holder">';
							html += '<h3>' + spsdev_mediamatic_translate.oops + '</h3>';
							html += '<p>' + spsdev_mediamatic_translate.folder_are_sub_directories + '</p>';
						html += '</div>';
						html += '<div class="links_holder">';
							html += '<a class="no" href="#">' + spsdev_mediamatic_translate.cancel + '</a>';
						html += '</div>';
					html += '</div>';
				html += '</div>';
			...
			$('#spsdev_mediamatic_be_confirm').remove();
			$('body').prepend(html);
			
			var confirm 		= $('#spsdev_mediamatic_be_confirm');
			confirm.addClass('opened folder_delete');
			var cancelActionBtn	= $('#spsdev_mediamatic_be_confirm').find('a.no');
		...
		errorPopup: function(){
			...
			var html = '<div id="spsdev_mediamatic_be_confirm">';
					html += '<div class="confirm_inner">';
						html += '<div class="desc_holder">';
							html += '<h3>' + spsdev_mediamatic_translate.error + '</h3>';
							html += '<p>' + spsdev_mediamatic_translate.error_occurred + '</p>';
						html += '</div>';
						html += '<div class="links_holder">';
							html += '<a class="yes green" href="#">' + spsdev_mediamatic_translate.reload + '</a>';
						html += '</div>';
					html += '</div>';
				html += '</div>';
			...
			var confirm 		= $('#spsdev_mediamatic_be_confirm');
			confirm.addClass('opened folder_delete');
			var cancelActionBtn	= $('#spsdev_mediamatic_be_confirm').find('a.yes');
		...
		deleteFolderConfirm: function(e_id){
			...
			var html = '<div id="spsdev_mediamatic_be_confirm">';
					html += '<div class="confirm_inner">';
						html += '<div class="desc_holder">';
							html += '<h3>' + spsdev_mediamatic_translate.are_you_sure + '</h3>';
							html += '<p>' + spsdev_mediamatic_translate.not_able_recover_folder + '</p>';
						html += '</div>';
						html += '<div class="links_holder">';
							html += '<a class="yes" href="#">' + spsdev_mediamatic_translate.yes_delete_it + '</a>';
							html += '<a class="no" href="#">' + spsdev_mediamatic_translate.cancel + '</a>';
						html += '</div>';
					html += '</div>';
				html += '</div>';
			...
			var confirm 		= $('#spsdev_mediamatic_be_confirm');
			confirm.addClass('opened folder_delete');
			var doActionBtn		= $('#spsdev_mediamatic_be_confirm').find('a.yes');
			var cancelActionBtn	= $('#spsdev_mediamatic_be_confirm').find('a.no');
		...
		limitationPopup: function(){
			...
			var html = '<div id="spsdev_mediamatic_be_confirm">';
					html += '<div class="confirm_inner">';
						html += '<div class="desc_holder">';
							html += '<h3>' + spsdev_mediamatic_translate.limit_folder_title + '</h3>';
							html += '<p>' + spsdev_mediamatic_translate.limit_folder_content + '</p>';
						html += '</div>';
						html += '<div class="links_holder">';
							html += '<a class="yes green" href="#">' + spsdev_mediamatic_translate.upgrade + '</a>';
							html += '<a class="no" href="#">' + spsdev_mediamatic_translate.no_thanks + '</a>';
						html += '</div>';
					html += '</div>';
				html += '</div>';
			...
			doActionBtn.off().on('click', function (e) {
				e.preventDefault();
				window.open(themedo_spsdev_mediamatic_pro_link, '_blank');
				$('#spsdev_mediamatic_be_confirm').remove();
				return false;
			});
			cancelActionBtn.on('click', function () {
        ...
        newBehavior: function () {
            ...
            $(document).on('click', '.add-new-folder-now', function (event) {
                ...
                if (name == '') {
                    alert(spsdev_mediamatic_translate.folder_name_enter);
                } else {
                    var parent = 0;
                    var depth = 0;
                    ...
                    mediamaticWMC.spsdev_mediamatic_begin_loading();

                    var data = {
                        'action': 'spsdev_mediamatic_ajax_update_folder_list',
                        'new_name': name,
                        'parent': parent,
                        'folder_type': 'default',
                        'type': 'new'
                    ...
                    $.post(ajaxurl, data, function (response) {
                        ...
                        spsdev_mediamatic_taxonomies.folder.term_list.push({ term_id: response.data.term_id, term_name: "new tmp folder" });
                        var $spsdev_mediamatic_sidebar = $('.spsdev_mediamatic_sidebar');
                        var backbone = mediamaticWMC.mediamaticWMCgetBackboneOfMedia($spsdev_mediamatic_sidebar);

                        if (typeof backbone.view === "object") {
                            var spsdev_mediamatic_Filter = backbone.view.toolbar.get("folder-filter");
                            if (typeof backbone.view === "object") {
                                spsdev_mediamatic_Filter.createFilters();
                            }
                        }
                        ...
                        mediamaticWMC.spsdev_mediamatic_finish_loading();
                    });
            ...
            $('#themedo-mediamatic-folderTree .jstree-anchor').dblclick(function (e) {
                e.preventDefault();
                setTimeout(function () {
                    $('.js_spsdev_mediamatic_rename').trigger('click');
                }, 100);
            });
            $('.js_spsdev_mediamatic_rename').on('click', function () {
        ...
        newFolderFormTemplate: function (extra_class) {
			...
			return '<li class="new-folder-wrap spsdev_mediamatic_new_and_edit ' + extra_class + '"><div class="input_holder"><img src="'+self.pluginUrl+'/assets/img/folder.svg" class="spsdev_mediamatic_be_svg" /><input type="text" name="add-new-folder-name" value="" class="folder-input add-new-folder-name" id="" autocomplete="off" /></div><div class="action_buttons"><span class="add-new-folder-now mf_confirm"><img src="'+self.pluginUrl+'/assets/img/check.svg" class="spsdev_mediamatic_be_svg" /></span><span class="add-new-folder-cancel mf_cancel"><img src="'+self.pluginUrl+'/assets/img/cancel.svg" class="spsdev_mediamatic_be_svg" /></span></div></li>';
        ...
        editFolderFormTemplate: function (id, val, extra_class) {
			...
			return '<li data-id="' + id + '" class="edit-folder-wrap spsdev_mediamatic_new_and_edit ' + extra_class + '"><div class="input_holder"><img src="'+self.pluginUrl+'/assets/img/folder.svg" class="spsdev_mediamatic_be_svg" /><input type="text" name="edit-folder-name" value="' + val + '" class="folder-input edit-folder-name" id="" autocomplete="off" /></div><div class="action_buttons"><span class="edit-folder-now mf_confirm"><img src="'+self.pluginUrl+'/assets/img/check.svg" class="spsdev_mediamatic_be_svg" /></span><span class="edit-folder-cancel mf_cancel" ><img src="'+self.pluginUrl+'/assets/img/cancel.svg" class="spsdev_mediamatic_be_svg" /></span></div></li>';
        },
        ...
        newFolderTemplate: function (id, name, parent, depth) {
            ...
            return '<li id="menu-item-' + id + '" data-id= "' + id + '" class="menu-item menu-item-depth-' + depth + '">' +
				...
				'<img src="'+self.pluginUrl+'/assets/img/folder.svg" class="spsdev_mediamatic_be_svg" />'+
